...
<head>
	...
	<script >
		function partCheck(){
			...
			location.href = "pList.pd?part="+part
		}
		function nWin(idx) {
...
<p><br/></p>
	<div class="container">
		...
		<div align = "right">
			<input type="button" value="자료업로드" class="btn btn-outline-secondary" onclick="location.href='${contextPath}/pInput.pd'"/>
		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach var="vo" items="${vos }">
					<tr >
						<td>${curScrNo }</td>
						<td onClick="nWin(${vo.idx})">${vo.title }
		...
		<ul class="pagination justify-content-center" >
			<c:if test="${pag != 1}">
    			<c:if test="${search != '' }">
					<li class="page-item"><a class="page-link" href="pList.pd?pag=${pag-1}&part=${part }&search=${search}&searchStr=${searchStr}">Previous</a></li>
				</c:if>	
				<c:if test="${search == '' }">
					<li class="page-item"><a class="page-link" href="pList.pd?pag=${pag-1}&part=${part }">Previous</a></li>
				</c:if>
    		</c:if>	
			...
			<c:forEach var="i" begin="0" end="${blockSize -1}" >
				<c:if test="${(startPageNum+i) <= totPage}">
					...
					<c:if test="${pag != (startPageNum+i)}">
						<c:if test="${search != '' }">
							<li class="page-item"><a class="page-link" href="${contextPath}/pList.pd?pag=${startPageNum+i}&part=${part }&search=${search}&searchStr=${searchStr}">${startPageNum+i}</a></li>
						</c:if>	
						<c:if test="${search == '' }">
							<li class="page-item"><a class="page-link" href="${contextPath}/pList.pd?pag=${startPageNum+i}&part=${part }">${startPageNum+i}</a></li>
						</c:if>					
					</c:if>
   				</c:if>
			</c:forEach> 			
			<c:if test="${pag != totPage}"> 
				<c:if test="${search != '' }">
					<li class="page-item"><a class="page-link" href="pList.pd?pag=${pag+1}&part=${part }&search=${search}&searchStr=${searchStr}">Next</a></li>
				</c:if>	
				<c:if test="${search == '' }">
					<li class="page-item"><a class="page-link" href="pList.pd?pag=${pag+1}&part=${part }">Next</a></li>
				</c:if>
    		</c:if>
    	</ul>	
  </div>
